...
// Función serverless de Vercel: hace el fetch a GeckoTerminal + CSWAP (Blockfrost)
// y guarda el resultado directo en el Cloudflare KV que usa el Worker.
//
// Variables de entorno necesarias en Vercel (Project Settings -> Environment Variables):
//   CF_ACCOUNT_ID        -> Account ID de Cloudflare
//   CF_KV_NAMESPACE_ID   -> e6c3b0a85920488984ca017a463dc510 (el mismo del wrangler.toml)
//   CF_API_TOKEN         -> el token con permiso Workers KV Storage:Edit
//   BLOCKFROST_PROJECT_ID -> el mismo project_id que ya usás en Cloudflare
//   CRON_SECRET          -> un secreto random para proteger este endpoint
...
const MAX_TRADES = 60;
const DELAY_MS = 2500; // milisegundos entre llamadas a GeckoTerminal (limite publico ~30 req/min = 1 cada 2s)
...
// WHALECOIN vive en bonding curve en snek.fun (todavía no graduó a un pool de DEX,
// asi que no aparece en GeckoTerminal). Usamos el mismo endpoint que consume el
// propio frontend de snek.fun (analytics.snek.fun) en vez de reconstruir swaps
// desde UTXOs de Blockfrost, que resultó frágil (direcciones de vault variables,
// órdenes en 2 pasos, etc.)
...
const WHALECOIN_UNIT = WHALECOIN_POLICY + WHALECOIN_ASSET_HEX; // sin punto, para nuestro formato interno
const SNEKFUN_ASSET_ID = `${WHALECOIN_POLICY}.${WHALECOIN_ASSET_HEX}`; // con punto, formato que espera snek.fun
const WHALECOIN_KV_KEY = 'whalecoin-cache.json';
const WHALECOIN_CACHE_TTL_MS = 15 * 60 * 1000; // 15 min, más corto que CSWAP porque no pega a Blockfrost (sin rate limit propio que cuidar)
const WHALECOIN_LIMIT = 60;
const WHALECOIN_MAX_AGE_MS = 12 * 60 * 60 * 1000; // no mostrar trades de WHALE de mas de 12hs
...
function parseSnekfunOrder(item) {
  // "." representa ADA en este API. Si "from" es ADA, el trader compró
  // WHALECOIN (buy); si "to" es ADA, el trader vendió WHALECOIN (sell).
  const isBuy = item.from === '.';
  const isSell = item.to === '.';
  if (!isBuy && !isSell) return null; // orden que no involucra ADA directo, ignorar

  const adaRaw = isBuy ? item.fromAmount : item.toAmount;
  const whaleRaw = isBuy ? item.toAmount : item.fromAmount;

  const adaAmount = Number(adaRaw) / 1_000_000; // lovelace -> ADA
  const tokenAmount = Number(whaleRaw); // WHALECOIN usa 0 decimales
...
export default async function handler(req, res) {
  ...
  // WHALECOIN va aparte porque no usa GeckoTerminal (esta en bonding curve, no
  // en un pool de DEX todavia). Si falla, no afecta a los demas tokens.
  ...
  // Dedupe: resultsBySymbol puede arrastrar trades viejos de WHALE desde el KV
  // de una corrida anterior (quedaron guardados bajo el symbol 'WHALE' la vez
  // pasada), y acá se vuelven a agregar frescos -> sin este paso quedan duplicados.
