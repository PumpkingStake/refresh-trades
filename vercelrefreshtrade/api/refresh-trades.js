...
const ADA_PRICE_KV_KEY = 'ada-usd-price.json';
const ADA_PRICE_CACHE_TTL_MS = 5 * 60 * 1000; // 5 min, CoinGecko free tier soporta esto de sobra
const MAX_TRADES = 60;
const DELAY_MS = 2500; 
...
// ============================================================
// Precio ADA/USD (CoinGecko, del lado del servidor)
// ============================================================
async function getAdaUsdPrice() {
  const cached = await kvGet(ADA_PRICE_KV_KEY);
  if (cached && Date.now() - cached.timestamp < ADA_PRICE_CACHE_TTL_MS) {
    console.log('📦 Usando caché de ADA/USD:', cached.price);
    return cached.price;
  }
  ...
  try {
    ...
    if (!isFinite(price) || price <= 0) throw new Error('precio inválido recibido');

    await kvPut(ADA_PRICE_KV_KEY, { price, timestamp: Date.now() });
    return price;
  } catch (error) {
    console.warn('⚠️ No se pudo obtener precio ADA/USD:', error.message, '- se usa el último valor cacheado.');
    if (cached && cached.price) return cached.price;
    return null; // sin caché previo y sin respuesta: el cliente decide qué hacer
  }
}
...
export default async function handler(req, res) {
  ...
  console.log('💵 Obteniendo precio ADA/USD...');
  const adaUsdPrice = await getAdaUsdPrice();

  const result = { trades, adaUsdPrice, updatedAt: Date.now() };
